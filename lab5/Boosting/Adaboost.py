...
class Adaboost:
    ...
    def fit(self, x_train, y_train):
        ...
        for t, weak_clf in enumerate(self.weak_clfs):
            ...
            errₜ = np.sum(W * Iₜ) / np.sum(W)  

            # TODO 5: Compute the estimator coefficient αₜ
            αₜ = np.log((1 - errₜ) / (errₜ + 1e-10))
            self.αs.append(αₜ)                  
    ...
    def predict(self, x_val):
        ...
        # Use zip
        weighted_opinions =  np.array([alpha * estimator.predict(x_val) for estimator, alpha in zip(self.weak_clfs, self.αs)])
